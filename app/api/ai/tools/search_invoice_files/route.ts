...
function matchesInvoice(name: string, invoice?: string) {
  if (!invoice) return true
  const inv = invoice.trim()
  return new RegExp(`\\b(invoice[-_ ]?)?${inv}\\b`, 'i').test(name) || name.toLowerCase().includes(inv.toLowerCase())
}
...
export async function POST(req: Request) {
  try {
    ...
    console.log(`Returning ${filesWithUrls.length} invoice files`)
